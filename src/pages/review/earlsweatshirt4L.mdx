---
title: "Earl Sweatshirt / Sick!"
description: "Album Review of Earl Sweatshirt / Sick!"
keywords: "Earl Sweatshirt, Sick!, アール・スウェットシャツ, シック！"
---

import { Slider, Button } from "@carbon/react";
import { ArrowUpRight } from "@carbon/icons-react";

import SliderJS1 from "../review/slider1";
import SliderJS2 from "../review/slider2";
import SliderJS3 from "../review/slider3";
import SliderJS4 from "../review/slider4";
import AdvJS2 from "../review/adv2";
import AdvJS3 from "../review/adv3";

import { Link } from "gatsby";

import Review1_1 from "../review/earlsweatshirt5.mdx";
import Review1_2 from "../review/earlsweatshirt3.mdx";
import Review2_1 from "../review/earlsweatshirt_alchemist1.mdx";

<p className="largeP">Album Review</p>
<h1 className="h1--no--margin">{props.pageContext.frontmatter.title}</h1>
<p className="largeP">
	<Link to="/best50/2022/">2022 Black Music Best No.14</Link>
</p>

<Row className="image-card-group">
	<Column colMd={3} colLg={4} noGutterMdLeft="">
		<ImageCard>

![Earl Sweatshirt / Sick!](../../images/cd/earlsweatshirt4L.jpg)

</ImageCard>
	</Column>
	<Column colMd={4} colLg={8} noGutterMdLeft="">
		<p>
			Earl Sweatshirtの4年ぶり、4作目のCD。この間、レーベル移籍を果たしWarnerからのリリースとなるが、音楽の方向性は前作を継承している。また、前作と同じく24分と収録時間は短めである。ただ、曲数が減ってるので各曲の特徴がでている。
			<br/>さらに、違いとしては、制作は全て外部Producerに任せたところで、その分、TrackのQualityは上がっている。ほぼ全編、ゆったりとして、リズムをずれ気味にしつつ、目いっぱい抑えた感じのTrackになっていて、つぶやくようなフローと一体になっている。
			<br/>Lyricも自身の思いを連ねたようなPersonalな感じで、ただ、今回はCovid19への言及が多数、見られている。
		</p>
		<div>
			<Button className="button-right-mergin" href="https://amzn.to/3GggCyZ" renderIcon={ArrowUpRight} size='sm' kind='primary'>
				amazon.com
			</Button>
			<Button className="button-right-mergin" href="https://amzn.to/3vfSeXL" renderIcon={ArrowUpRight} size='sm' kind='secondary'>
				amazon.co.jp
			</Button>
			<Button className="button-right-mergin" href="https://apple.co/3Ghm04P" renderIcon={ArrowUpRight} size='sm' kind='secondary'>
				apple music
			</Button>
			<AdvJS2/>
		</div>
	</Column>
</Row>
<Row>
	<Column colMd={4} colLg={4} noGutterMdLeft="">
		<div>
			<h3>Score card</h3>
			<SliderJS1 value="1" />
			<SliderJS2 value="2" />
			<SliderJS3 value="2" />
			<SliderJS4 value="8" />
		</div>
	</Column>
	<Column colMd={8} colLg={8} noGutterMdLeft="">
		<div>
			<h3>Producers</h3>
			<p>
				The Alchemist(1,6)
				<br/>Black Noi$e(2,4,9,10)
				<br/>Ancestors(3)
				<br/>Bob Chambers and Theravada(5)
				<br/>Samiyam(7)
				<br/>Alexander Spit(8)
			</p>
			<h3>Guests</h3>
			<p>
				Zelooperz, Armand Hammer
			</p>
		</div>
	</Column>
</Row>

<h3>Tracks</h3>

| No. | Title            | Composers                                                                         | Performer                          | Time  |
| --- | ---------------- | --------------------------------------------------------------------------------- | ---------------------------------- | ----- |
| 1   | Old Friend       | Thebe Kgositsile / Alan Maman                                                     | Earl Sweatshirt                    | 01:19 |
| 2   | 2010             | Thebe Kgositsile / Robert Mansell                                                 | Earl Sweatshirt                    | 02:28 |
| 3   | Sick!            | Sage Elsesser / Thebe Kgositsile                                                  | Earl Sweatshirt                    | 01:51 |
| 4   | Vision           | Thebe Kgositsile / Robert Mansell / Walter Williams                               | Earl Sweatshirt feat. Zelooperz    | 04:14 |
| 5   | Tabula Rasa      | Chaz Hall / Thebe Kgositsile / Thaniil Theoharis / Billy Woods / Xenophon Yialias | Earl Sweatshirt feat. Armand Hammer | 04:11 |
| 6   | Lye              | Thebe Kgositsile / Alan Maman                                                     | Earl Sweatshirt                    | 01:52 |
| 7   | Lobby (Int)      | Thebe Kgositsile / Sam Baker                                                                       | Earl Sweatshirt                    | 01:12 |
| 8   | God Laughs       | Thebe Kgositsile / Alexander Manzano                                              | Earl Sweatshirt                    | 01:24 |
| 9   | Titanic          | Thebe Kgositsile / Robert Mansell                                                 | Earl Sweatshirt                    | 01:52 |
| 10  | Fire in the Hole | Thebe Kgositsile / Robert Mansell                                                 | Earl Sweatshirt                    | 03:37 |

<h3>Other Reviews</h3>

<Row>
	<Column colMd={3} colLg={3} noGutterMdLeft>
		<Review1_1 />
	</Column>
	<Column colMd={3} colLg={3} noGutterMdLeft>
		<Review1_2 />
	</Column>
</Row>

<Row>
	<Column colMd={3} colLg={3} noGutterMdLeft>
		<Review2_1 />
	</Column>
</Row>

<AdvJS3/>