...
if (!inputFile) {
  console.error('Usage: node fetch_apple_music_credits.js <input-json-file>');
  console.error('Example: node fetch_apple_music_credits.js apple-music-view-details-urls.json');
  process.exit(1);
}
...
if (!jsonData.trackUrls || !Array.isArray(jsonData.trackUrls)) {
  console.error('Error: Invalid JSON structure. Expected "trackUrls" array.');
  process.exit(1);
}
...
async function processAllTracks() {
  const albumTitle = jsonData.albumTitle || 'Unknown Album';
  const albumArtist = jsonData.albumArtist || 'Unknown Artist';
  
  console.log(`Processing ${jsonData.trackUrls.length} tracks from "${albumTitle}" by ${albumArtist}...\n`);
  ...
  const outputBasename = `${inputBasename}-credits`;
  ...
  let markdown = `# ${albumTitle} - Track Details\n\n`;
  markdown += `**Artist:** ${albumArtist}\n\n`;
  if (jsonData.albumUrl) {
  ...
  let textList = `${albumTitle} by ${albumArtist}\n`;
  textList += `${'='.repeat(60)}\n\n`;
