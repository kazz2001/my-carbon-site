...
最後にBOB\review_refine_prompt.mdの19行目に、File1を上書きで書き込んで。ほかの行は変更しないで。

# 2 
src\pages\review\dvsn1L.mdx
を整形して。インデントはタブに統一。テーブルの乱れも直してね。

# 3
src\pages\review下で、先頭だ"g"で終わりがL.mdxの全ファイルを整形して。インデントはタブだよ。
テーブルの乱れも直してね。
修正したファイルのファイル名だけをBOB\review_refine_prompt.mdの23行目以降に、上書きで書き込んで。ほかの行は変更しないで。

# 成形
k

# 5
src\pages\review\kelela3L.mdx
...
import Review1 from "../review/luckydaye2.mdx";
